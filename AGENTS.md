...
- 言語切替を押すと選択を `localStorage` の `preferred-locale` に覚える
  - 覚えた言語を使うのは `/` に来たときだけ。`Head.astro` のインラインscriptが送る
  - 個別ページではリダイレクトしない。共有されたURLはそのURLの言語のまま開く
  - 切替ボタンには `data-language-switch` を付ける。増やすときも同じ属性で拾う
